...
const { run, get, all, serializedWrite, transaction } = require('../db/database');
...
router.get('/', (req, res) => {
  ...
  `;
  const params = [];
  if (warehouse_id) { sql += ' AND a.warehouse_id = ?'; params.push(warehouse_id); }
...
router.get('/stats', (req, res) => {
  const unresolved = get('SELECT COUNT(*) as cnt FROM stock_alerts WHERE is_resolved = 0');
  const byWarehouse = all(`
...
router.put('/:id/resolve', async (req, res) => {
  try {
    await serializedWrite(() => {
      ...
      if (!alert) throw new Error('预警记录不存在');
      run('UPDATE stock_alerts SET is_resolved = 1 WHERE id = ?', [req.params.id]);
    });
    res.json({ success: true, message: '预警已处理' });
  } catch (err) {
    res.status(/不存在/.test(err.message) ? 404 : 500).json({ success: false, message: err.message });
  }
});
...
router.post('/scan-all', async (req, res) => {
  ...
  await serializedWrite(() => {
    ...
    for (const inv of inventory) {
      const alert = checkAndCreateLowStockAlert(inv.product_id, inv.warehouse_id);
